class Accessor {
  ...
  create = async (obj) => {
    ...
    const {isSuccess, result, message } = await this.read(status.insertId);
      ...
      return { isSuccess: false, result: null, message: `Failed to recover inserted record: ${error}` };
    }
  };
  ...
  update = async (id, obj) => {
    ...
    const {isSuccess, result, message } = await this.read(id);
      ...
      return { isSuccess: false, result: null, message: `Failed to recover update record: ${error}` };
    }
  };
